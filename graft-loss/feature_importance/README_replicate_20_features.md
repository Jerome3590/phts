...
## Data Source and Censoring Implementation

### Using `phts_txpl_ml.sas7bdat`

The script uses **`phts_txpl_ml.sas7bdat`** to match the original study, which includes proper censoring implementation:

- **File**: `data/phts_txpl_ml.sas7bdat` (matches original study)
- **Censoring handling**: The original study's `clean_phts()` function includes censoring implementation that:
  - Sets event times of 0 to 1/365 (prevents invalid zero times)
  - Properly handles censored observations for survival analysis
- **Why this file**: The original Wisotzkey study used `phts_txpl_ml.sas7bdat` specifically because it has the censoring implementation needed for accurate survival modeling

### Censoring Details

The censoring implementation ensures:
1. **No zero event times**: Event times of 0 are replaced with 1/365 (1 day) to prevent numerical issues
2. **Proper survival structure**: Censored observations (status = 0) are correctly maintained throughout the analysis
3. **Consistent with original study**: Matches the exact data processing used in Wisotzkey et al. (2023)

**Note**: While `transplant.sas7bdat` exists and has the same structure, `phts_txpl_ml.sas7bdat` is used to ensure exact replication of the original study's censoring handling.
...
2. Verify `data/phts_txpl_ml.sas7bdat` exists (required - no fallback)
3. The script will fail with a clear error if the file is not found
