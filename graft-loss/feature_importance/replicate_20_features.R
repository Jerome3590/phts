...
# Configuration
n_predictors <- 20  # Target: select top 20 features from ALL variables (matching original study)
...
prepare_modeling_data <- function(data) {
  ...
  # Keep ALL variables (matching original study: feature selection from all variables)
  # Remove only ID columns and outcome/leakage variables that shouldn't be used as predictors
  # Keep time and status for survival modeling
  ...
  #     Examples: dtx_patient (donor transplant patient status - post-event)
  #
  ...
  #   - dcardiac, dneuro, dreject, dsecaccs, dpriaccs: Donor complications
  #   - dconmbld, dconmal, dconcard, dconneur, dconrej: Donor complication categories
  #   - dmajbld, dmalcanc: Donor major complications
  #   - All variables starting with "dcon", "dpri", "dsec", "dmaj": Donor post-transplant
  #
  ...
  exclude_exact <- c(
    ...
    "deathspc",  # Death specific cause (post-event)
    "concod",  # Cause of death (post-event)
    # Patient support/status variables (unclear timing - likely post-event)
    "patsupp",  # Death: VAD/ECMO (post-event)
    "pmorexam",  # Death: Post Mortem Exam (post-event)
    "papooth",  # Death: Cardiac pathology: Other (post-event)
    "pacuref",  # Death: Cardiac pathology: Acute rejection (post-event)
  ...
  exclude_prefixes <- c(
    "dtx_",   # Donor/transplant post-event (already documented)
...
analyze_time_period <- function(period_name, period_data) {
  ...
  # Prepare data (use ALL variables, matching original study workflow)
