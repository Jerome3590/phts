...
# ===================
# ORCHESTRATOR: Run Complete Pipeline for Multiple Datasets
# ===================
# This script launches three complete pipeline runs in parallel:
# 1. Original Study (2010-2019)
# 2. Full Dataset with COVID
# 3. Full Dataset without COVID
# 
# Each pipeline runs steps 00-05 independently with proper resource management.
...
cat("==== Pipeline Orchestrator: Multiple Dataset Runs ===\n")
...
for (nm in names(runs)) {
  ...
  # Use per-model, per-split log file structure for each dataset
  cohort_name <- nm
  label_name <- "full"  # Or set dynamically if needed
  
  # CONSISTENT LOG FILE LOCATION STRUCTURE:
  # logs/models/{cohort_name}/{label_name}/orchestrator.log
  # Example: logs/models/original/full/orchestrator.log
  log_dir <- file.path("logs", "models", cohort_name, label_name)
  dir.create(log_dir, showWarnings = FALSE, recursive = TRUE)
  log_file <- file.path(log_dir, "orchestrator.log")

  # Enhanced logging: wrap Rscript call to redirect output to orchestrator log
  # All output and errors for this dataset go to orchestrator.log
  cmd <- sprintf("nohup %s scripts/enhanced_pipeline_logger_v2.R %s %s > %s 2>&1 & echo $!", 
                 shQuote(rscript), shQuote(nm), shQuote(log_file), shQuote(log_file))
  ...
  if (length(pid) > 0 && nzchar(pid)) {
    ...
    # Write status/progress to orchestrator log
    cat(sprintf("[LAUNCH] %s | PID %s | %s\n", nm, pid, format(Sys.time(), "%Y-%m-%d %H:%M:%S")), file = log_file, append = TRUE)
  } else {
