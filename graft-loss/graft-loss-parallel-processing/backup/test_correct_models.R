# Correct Model Test Script
# This tests all models with proper prediction handling based on the actual function signatures
...
cat("[test_correct_models.R] Starting correct model test\n")

# Load the simplified dataset
phts_data <- readRDS(here("model_data", "phts_simple.rds"))
cat(sprintf("[test_correct_models.R] Loaded dataset: %d rows, %d columns\n", 
...
cat(sprintf("[test_correct_models.R] Train: %d rows, Test: %d rows\n", 
...
cat("[test_correct_models.R] Data preparation complete\n")
...
tryCatch({
  ...
  # For XGBoost, we need to use the correct prediction method
  # The fit_xgb function expects newdata to be a data frame with the same structure
  te_df_xgb <- te_df[, wisotzkey_features, drop = FALSE]
  
  # Use the predict method directly on the model
...
tryCatch({
  ...
  # For CPH, we need to use the correct prediction method
  # The fit_cph function expects newdata to be a data frame with the same structure
  te_df_cph <- te_df[, wisotzkey_features, drop = FALSE]
  
  # Use the predict method directly on the model
  cph_pred <- predict(cph_model, newdata = te_df_cph, pred_horizon = 1)
...
cat("\n[test_correct_models.R] Correct model test completed\n")
