# Final Working Model Test Script
# This tests all models with proper prediction handling
...
cat("[test_final_models.R] Starting final model test\n")

# Load the simplified dataset
phts_data <- readRDS(here("model_data", "phts_simple.rds"))
cat(sprintf("[test_final_models.R] Loaded dataset: %d rows, %d columns\n", 
...
cat(sprintf("[test_final_models.R] Train: %d rows, Test: %d rows\n", 
...
cat("[test_final_models.R] Data preparation complete\n")
...
tryCatch({
  ...
  # Create a proper data frame for XGBoost prediction
  te_df_xgb <- as.data.frame(te_df_xgb)
...
tryCatch({
  ...
  # Create matrix with only the predictor variables, ensuring all are numeric
  te_matrix <- data.matrix(te_df_cph[, wisotzkey_features, drop = FALSE])
  cat(sprintf("CPH test matrix: %d rows, %d cols\n", nrow(te_matrix), ncol(te_matrix)))
  cat(sprintf("CPH matrix class: %s\n", class(te_matrix)))
  cat(sprintf("CPH matrix mode: %s\n", mode(te_matrix)))
...
cat("\n[test_final_models.R] Final model test completed\n")
