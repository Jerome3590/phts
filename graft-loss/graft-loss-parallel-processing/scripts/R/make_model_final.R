...
make_model_final <- function(phts_all) {
  ...
  for(v in seq_along(variable_values)){
    ...
    # use safe predict helper to handle ranger/orsf/cph differences
    partial$prediction[[v]] <- safe_model_predict(model, newdata = trn_pd, times = 1)
  ...
  for(.name in unique(partial$name)) {
    ...
    for(.variable in unique(partial$variable[partial$name == .name])){
      ...
      partial$values[[counter]] <- as.numeric(
        1 - safe_model_predict(model, newdata = trn_pd, times = 1)
      )
  ...
  cph <- survival::coxph(Surv(time, status) ~ .,
