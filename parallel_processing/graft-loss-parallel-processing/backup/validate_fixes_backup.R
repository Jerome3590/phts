...
config_path <- file.path("scripts", "config.R")
...
tryCatch({
  ...
  setup_content <- readLines(file.path("scripts", "00_setup.R"))
  source_line <- grep("source.*config", setup_content, value = TRUE)
  if (length(source_line) > 0 && grepl("scripts.*config", source_line[1])) {
...
required_dirs <- c("scripts", "R", "R/utils")
missing_dirs <- c()
...
smart_setup_path <- file.path("scripts", "smart_setup.R")
