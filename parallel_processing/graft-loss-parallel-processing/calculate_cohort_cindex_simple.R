#!/usr/bin/env Rscript
# Direct C-index calculation for cohorts - Simple version
# Works with existing pipeline data structure
...
cat("Available columns:", paste(names(model_data), collapse = ", "), "\n")
...
define_cohorts <- function(data) {
  ...
  if (length(year_cols) > 0) {
    ...
    cohorts$non_covid_full <- data %>% filter(!!sym(year_col) < 2020 | !!sym(year_col) > 2021)
  } else {
...
# Define survival lagging keywords (simplified)
