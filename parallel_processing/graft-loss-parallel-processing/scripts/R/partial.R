...
.partial <- function(model, data, variable_name, variable_values){
  ...
  safe_predict_vec <- function(df) {
    out <- tryCatch({
      # Use central safe_model_predict which handles ranger/rfsrc/orsf/xgb differences
      p <- safe_model_predict(model, newdata = df, times = 1)
      ...
      p <- tryCatch(safe_model_predict(model, newdata = df), error = function(e2) NA_real_)
      if (is.list(p) && !is.null(p$predictions)) p <- p$predictions
      if (is.matrix(p)) p <- p[, 1, drop = TRUE]
