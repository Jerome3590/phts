...
def fcnet(x_, o_dim_, o_fn_, num_layers_=1, h_dim_=100, activation_fn=tf.nn.relu, keep_prob_=1.0, w_reg_=None, name='fcnet', reuse=tf.compat.v1.AUTO_REUSE):
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse):
        if num_layers_ == 1:
            out =  tf.compat.v1.layers.dense(inputs=x_, units=o_dim_, activation=o_fn_, kernel_regularizer=w_reg_)
        else:
            for tmp_layer in range(num_layers_-1):
                if tmp_layer == 0:
                    net = x_
                net = tf.compat.v1.layers.dense(inputs=net, units=h_dim_, activation=activation_fn, kernel_regularizer=w_reg_)
                net = tf.nn.dropout(net, rate=1 - (keep_prob_))
            out =  tf.compat.v1.layers.dense(inputs=net, units=o_dim_, activation=o_fn_, kernel_regularizer=w_reg_)  
    return out
...
#             self.z = tf.math.l2_normalize(self.z, axis=0)
            ...
            if self.ipm_term != 'no_ipm':
                ...
                for m in range(0, self.t_max):
                    ...
                    if self.is_treat:
                        idx0             = tf.compat.v1.where(tf.equal((1.-self.a[:, 0])*self.mask2[:, m], 1.))[:,0]
            ...
            if self.reg_scale != 0:
                ...
                regularizer       = tf.keras.regularizers.l2(l=0.5 * (self.reg_scale))
                loss_reg          = tf.losses.get_regularization_loss(regularizer,vars_reg)
