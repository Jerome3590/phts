---
title: "Survival Analysis Models - Unified Cohort"
...
def train_pycox_models(data_dir, cohort_name, metrics_path='metrics/pycox_metrics.csv', epochs=128, bs=128):
    # Reproducibility
    random.seed(1997)
    np.random.seed(1997)
    ...
    try:
        ...
        pd.DataFrame({
            ...
            'metrics/pycox_deepsurv_importance.csv', index=False
        )
    ...
    try:
        ...
        # DeepHit importance (proxy: first Linear layer weights)
        try:
            first_linear_dh = next((m for m in net_dh.modules() if isinstance(m, nn.Linear)), None)
            if first_linear_dh is None:
                raise RuntimeError('no Linear layer found')
            w_dh = first_linear_dh.weight.detach().cpu().numpy()     # [hidden, in_features]
            importances_dh = np.abs(w_dh).sum(axis=0)                # per-input feature importance
            pd.DataFrame({
                'feature': feature_names,
                'importance': importances_dh,
                'Model': 'DeepHit (pycox)',
                'Cohort': cohort_name
            }).sort_values('importance', ascending=False).to_csv(
                'metrics/pycox_deephit_importance.csv', index=False
            )
        except Exception as e:
            print('DeepHit importance export skipped:', e)
    ...
    try:
        ...
        if rows:
            ...
            risk_path = 'metrics/pycox_risk_scores.csv'
...
train_pycox_models('pycox_full', 'Full Dataset')
...
dl_imp_files <- list(
  deepsurv = 'metrics/pycox_deepsurv_importance.csv',
  deephit  = 'metrics/pycox_deephit_importance.csv'
)
...
if (file.exists('metrics/pycox_risk_scores.csv')) {
  pycox_risk <- readr::read_csv('metrics/pycox_risk_scores.csv', show_col_types = FALSE)
  ...
  for (mdl in unique(pycox_risk$Model)) {
    # Take the latest record per idx for current cohort/model to avoid concat duplicates
    n_test <- nrow(test)
    vec_df <- pycox_risk |>
      dplyr::filter(Cohort == "Full Dataset", Model == mdl, idx >= 0, idx < n_test) |>
      dplyr::group_by(idx) |>
      dplyr::slice_tail(n = 1) |>
      dplyr::ungroup() |>
      dplyr::arrange(idx)
    if (nrow(vec_df) < n_test) next

    vec <- vec_df |>
      dplyr::pull(risk)

    # Align by idx (0-based in CSV) to test rows (1-based in R)
    pos <- as.integer(vec_df$idx) + 1L
    pos <- pos[pos >= 1 & pos <= n_test]
    if (!length(pos)) next

    time_aligned <- as.numeric(test$time[pos])
    status_aligned <- as.integer(test$status[pos])

    # Clean NAs/infinites across all aligned vectors
    finite_idx <- is.finite(vec) & is.finite(time_aligned) & !is.na(status_aligned)
    if (!any(finite_idx)) next
    vec_use <- as.numeric(vec[finite_idx])
    # Ensure higher = higher risk for concordance. For DeepSurv (pycox), predicted partial hazards
    # tend to be higher risk when larger already; if direction seems inverted (Harrell < 0.5), flip.
    if (mdl == 'DeepSurv (pycox)') {
      har_tmp <- tryCatch({
        as.numeric(survival::concordance(survival::Surv(time_aligned[finite_idx], status_aligned[finite_idx]) ~ vec_use)$concordance)
      }, error = function(e) NA_real_)
      if (is.finite(har_tmp) && !is.na(har_tmp) && har_tmp < 0.5) vec_use <- -vec_use
    }
    time_use <- as.numeric(time_aligned[finite_idx])
    status_use <- as.integer(status_aligned[finite_idx])
    if (length(vec_use) < 2) next

    # Pick a safe tau within both train event horizon and test follow-up
    te_max <- suppressWarnings(max(time_use, na.rm = TRUE))
    tr_event_max <- suppressWarnings(max(train$time[train$status == 1], na.rm = TRUE))
    q90 <- suppressWarnings(as.numeric(stats::quantile(time_use, 0.9, na.rm = TRUE)))
    cands <- c(1, q90, te_max, tr_event_max)
    tau_use <- suppressWarnings(min(cands[is.finite(cands) & !is.na(cands)], na.rm = TRUE))
    # Nudge below boundary to avoid edge issues in IPCW
    eps <- 1e-6
    if (is.finite(tau_use) && !is.na(tau_use)) {
      tau_use <- max(eps, tau_use - eps)
    } else {
      tau_use <- 1
    }
    ...
    c_pair <- compute_concordance_pair(
      train_time   = train$time, train_status = train$status,
      test_time    = time_use,   test_status  = status_use,
      risk         = vec_use,
      tau          = tau_use
    )
...
```{r dl-concordance-debug, echo=TRUE, warning=FALSE, message=TRUE, eval=TRUE}

# Debug walkthrough for Deep Learning Harrell/Uno computation
if (file.exists('metrics/pycox_risk_scores.csv')) {
  pycox_risk_dbg <- readr::read_csv('metrics/pycox_risk_scores.csv', show_col_types = FALSE)

  models_to_check <- intersect(unique(pycox_risk_dbg$Model), c('DeepSurv (pycox)', 'DeepHit (pycox)'))
  if (!length(models_to_check)) models_to_check <- unique(pycox_risk_dbg$Model)

  for (mdl in models_to_check) {
    cat('\n=== DL Concordance Debug for:', mdl, '===\n')
    n_test <- nrow(test)

    # Take the latest record per idx (avoid duplicates from appends), current cohort
    vec_df <- pycox_risk_dbg |>
      dplyr::filter(Cohort == 'Full Dataset', Model == mdl, idx >= 0, idx < n_test) |>
      dplyr::group_by(idx) |>
      dplyr::slice_tail(n = 1) |>
      dplyr::ungroup() |>
    ...
    cat('Rows for model after grouping by idx:', nrow(vec_df), '\n')
    if (nrow(vec_df) < n_test) { cat('Insufficient rows to align with test. Skipping.\n'); next }

    # Align by idx (0-based to 1-based)
    pos <- as.integer(vec_df$idx) + 1L
    pos <- pos[pos >= 1 & pos <= n_test]
    if (!length(pos)) { cat('No valid positions after idx alignment.\n'); next }

    vec <- as.numeric(vec_df$risk)
    time_aligned <- as.numeric(test$time[pos])
    status_aligned <- as.integer(test$status[pos])

    # Clean to finite entries across all
    keep <- is.finite(vec) & is.finite(time_aligned) & !is.na(status_aligned)
    vec_use <- vec[keep]
    time_use <- time_aligned[keep]
    status_use <- status_aligned[keep]

    cat('Aligned length:', length(vec_use), '| Events in test aligned:', sum(status_use == 1, na.rm = TRUE), '\n')
    cat('Risk sd:', stats::sd(vec_use), '| Unique risks:', length(unique(vec_use)), '\n')
    print(summary(time_use))
    print(table(status_use, useNA = 'ifany'))

    if (length(vec_use) < 2 || all(vec_use == vec_use[1])) {
      cat('Risk scores constant or too few points; Uno will likely be NA.\n')
    }

    # Harrell
    harrell_dbg <- tryCatch({
      as.numeric(survival::concordance(survival::Surv(time_use, status_use) ~ vec_use)$concordance)
    }, error = function(e) NA_real_)
    cat('Harrell C (debug):', round(harrell_dbg, 4), '\n')

    # Tau candidates and Uno per candidate
    tr_event_max <- suppressWarnings(max(train$time[train$status == 1], na.rm = TRUE))
    te_max <- suppressWarnings(max(time_use, na.rm = TRUE))
    q90 <- suppressWarnings(as.numeric(stats::quantile(time_use, 0.9, na.rm = TRUE)))
    cands <- c(`1yr` = 1, `q90(test)` = q90, `max(test)` = te_max, `max(train_event)` = tr_event_max)
    eps <- 1e-6
    out_tau <- lapply(names(cands), function(nm) {
      base <- cands[[nm]]
      if (!is.finite(base) || is.na(base) || base <= 0) return(data.frame(name = nm, tau = NA_real_, Uno = NA_real_))
      tau_try <- min(base, te_max, tr_event_max, na.rm = TRUE) - eps
      if (!is.finite(tau_try) || is.na(tau_try) || tau_try <= 0) return(data.frame(name = nm, tau = NA_real_, Uno = NA_real_))
      uno_val <- tryCatch({
        if (!requireNamespace('survAUC', quietly = TRUE)) stop('survAUC missing')
        survAUC::UnoC(
          survival::Surv(train$time, train$status),
          survival::Surv(time_use, status_use),
          marker = vec_use,
          tau = tau_try
        )$C
      }, error = function(e) NA_real_)
      data.frame(name = nm, tau = tau_try, Uno = uno_val)
    })
    tau_tbl <- dplyr::bind_rows(out_tau)
    print(tau_tbl)
  }
} else {
  cat('No pycox_risk_scores.csv found; cannot run DL concordance debug.\n')
}
...
if (length(all_metrics_list) > 0) {
  ...
  if (file.exists('metrics/pycox_metrics.csv')) {
    pycox_df <- readr::read_csv('metrics/pycox_metrics.csv', show_col_types = FALSE)
  ...
  # Persist metrics
  readr::write_csv(metrics_df, file.path('metrics', 'metrics_full.csv'))
} else {
...
if (exists("metrics_df")) {
  ...
  out_path <- file.path('metrics', 'best_model_full.csv')
...
# 6. DeepSurv (pycox)
if (file.exists('metrics/pycox_deepsurv_importance.csv')) {
  deepsurv_df <- readr::read_csv('metrics/pycox_deepsurv_importance.csv', show_col_types = FALSE)
...
# 7. DeepHit (pycox)
if (file.exists('metrics/pycox_deephit_importance.csv')) {
  deephit_df <- readr::read_csv('metrics/pycox_deephit_importance.csv', show_col_types = FALSE)
...
if (exists("all_features_df") && !is.null(all_features_df) && nrow(all_features_df) > 0) {
  ...
  readr::write_csv(feature_summary_df, file.path('metrics', 'feature_importance_full.csv'))
} else {
...
if (exists("sankey_plot")) {
  htmlwidgets::saveWidget(
    sankey_plot,
    file = "sankey_time_to_event_unified_cohort_feature_importance.html",
    selfcontained = TRUE
  )
}
